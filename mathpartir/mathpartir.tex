...
\def \macro #1{\texttt{\string #1}\xspace}
...
The first part is an environment \texttt{mathpar} that generalizes the math
...
Although \texttt{mathpar} and \texttt{inferrule} look similar in their
...
\end{tabular}
$$
Formulas are separated by \macro\and (or equivalently by a blank line).
To enforce a vertical break it suffices to replace \macro\and by
\verb"\\".

The implementation of \texttt{mathpar} entirely relies on the paragraph mode
...
\macro\and to something like \macro\goodbreak.

Since \texttt{version 1.5.0}, the environment \texttt{mathpar}
reduces the vertical skip using \TeX\ variable
\macro\abovedisplayshortskip instead of \macro\abovedisplayskip
when the first line of the display is short enough not to overlap with the
last line before the display. For instance, when this is activated, you may
compare this short line:
\shortmathpartrue
\begin{mathpar}
one
...
two
\end{mathpar}
preceding the display, with this much longer last line: 
\begin{mathpar}
one
...
\end{mathpar}
%% $$
%% \hbox to \linewidth {\hfil foo \hfil \two}
%% $$
This behavior can be set automatically with the package option
\texttt{shortdisplay}, which is currently not set by default.
It can be locally activated (resp. deactivated) with boolean
\macro \shortmathparfalse (resp. \macro \shortmathpartrue). 
When the option is deactivated a short display following a short line would
be displayed\linebreak as: 
%%\HEVEA \renewcommand{\hva@cellskip}{10em}
\shortmathparfalse %\shortmathpartrue
\begin{mathpar}
one

two
\end{mathpar}
with a full \macro\displayskip instead of a
\macro\shortdisplayskip amount. 

An alternative environment \texttt{mathparpagebreakable} may also be used to
allow for page breaks in the middle of the display.  This however cancels the
previous optimization for short displays.
...
\texttt{keyval} package, so as to offer a set of record-like options:
$$
...
\\\hline
narrower & d & Set the width of the rule to $d$ times \macro\hsize.
\\\hline
...
\\\hline
right & \ell & As \texttt{left},  but on the right of the rule.
\\\hline
...
\macro\inferrule as described in table below.
...
Notice, that if the package \texttt{hyperref} is loaded, one can automatically
attach hypertargets to rule definitions names of rules defined with the
non-star version of \macro\inferrule and refers to them by
...
The package also defines \macro\infer as a shortcut for \macro\inferrule
...
There is also a version \macro\triplerule originally design to print
...
\macro\hsize is 8em to folow its typesetting on several lines).
\begin{lstlisting}{Ocaml}
\triplerule[Hoare]
    {ppppp \\  pppp \\ pppp}
    {mmm \\ mmm \\ mm}
...
\texttt{hbox} for typesetting each line and one \texttt{vbox} for collecting
lines. The premise appears as a list with
\verb"\\" as separator. Each element is considered in turn typeset in a
\texttt{hbox} in display math mode. Its width is compare to the space left on
...
The vertical space in \texttt{mathpar} is adjusted by
...
The environment uses \verb"\MathparBindings" to rebind \verb"\\",
\macro \and, and \texttt{\char`\\par}. You can redefine thus command to
change the default bindings or add your own.
...
The package also redefines \macro\hva to do nothing in \LaTeX mode in
both \lst"mathpar" environment nor in inference rules

In \hevea mode and \texttt{mathpar} environment, \macro\and will always
...
\section {Testing for short display}

\shortmathpartrue%\shortmathparfalse
...
%% LocalWords: mathpar aa Yop bb dd ee ff cc Ocaml Foo leftskip centerline
%% LocalWords: rightskip vdots gg hh uu vv ww INRIA Didier shortdisplay bbc
%% LocalWords: HeVeA escapechar inferrule yop flushleft bbb minipage
...
%% LocalWords: hbox vbox mathpartir mathparpagebreakable uncapitalized
% LocalWords:  hyperref
